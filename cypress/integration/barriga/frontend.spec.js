...
describe('Should test at a functional level', () => {
    after(() => { //Comando oposto ao before() 
        cy.clearLocalStorage() //Limpa o Local Storage do navegador relacionado a um determinado domínio.
        //Ele fez isso porque cada vez que faz-se login na aplicação, os dados do usuário logado ficam salvos no storage, e para testes isso não é interessante
        //Entre um teste e outro ele faz essa limpeza, mas rodando o mesmo teste de novo ele não limpa
    })
    ...
    it('should test the responsiveness', () => {
        cy.get('[data-test=menu-home]').should('exist')
            .and('be.visible')
        cy.viewport(500, 700) //trocar tamanho da tela
        cy.get('[data-test=menu-home]').should('exist')
            .and('be.not.visible')
    ...
    it('Should update an account', () => {
        cy.route({
            method: 'PUT',
            url: '/contas/**', //Com os ** ele aceita qualquer coisa que venha depois de /contas/
    ...
    it('Should create a transaction', () => {
        ...
        cy.route({
            method: 'GET',
            url: '/extrato/**',
            response: 'fixture:movimentacaoSalva' //Ele precisou colocar especificar que o arquivo movimentacaoSalva é uma fixture
        })
    ...
    it.skip('Should validate data send to create an account', () => {
        ...
        cy.wait('@saveConta').then(() => {
            ...
            expect(reqStub.args[0][0].request.body.nome).to.be.empty //Que não deve vir com valor vazio
